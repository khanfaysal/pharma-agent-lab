...
export default function SearchPage() {
  ...
  return (
    <div className="space-y-6">
      <header>
        <h1 className="font-mono text-2xl font-bold uppercase tracking-tight text-navy-900">Search</h1>
      ...
      <form onSubmit={(e) => { e.preventDefault(); void search(); }} className="card-dev space-y-3 p-4">
        ...
        <div className="flex items-center gap-2">
          ...
          {latency !== null && <span className="ml-auto text-xs text-slate-400">{latency} ms</span>}
        </div>
      </form>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <section className="space-y-2">
          ...
          {docs?.length === 0 && (
            <p className="card-dev p-4 text-sm text-slate-400">No passages matched.</p>
          )}
          {docs?.map((chunk) => (
            <article key={chunk.chunkId} className="card-dev p-3">
              <div className="flex items-baseline justify-between gap-2">
                <h3 className="text-sm font-medium text-navy-900">{chunk.title}</h3>
              ...
              <p className="mt-2 line-clamp-6 whitespace-pre-wrap text-xs leading-relaxed text-slate-600">
                {chunk.content}
              </p>
            </article>
          ))}
        ...
        <section className="space-y-2">
          ...
          {brands?.length === 0 && (
            <p className="card-dev p-4 text-sm text-slate-400">No brands matched.</p>
          )}
                  ...
                  {brands.map((b) => (
                    <tr key={b.brand_id}>
                      ...
                      <td className="td text-right font-mono text-xs">
                        {b.price_min ?? '—'}
                        {b.packsize && <div className="text-slate-400">{b.packsize}</div>}
                      </td>
                    </tr>
                  ))}
                </tbody>
