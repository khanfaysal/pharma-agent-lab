  ...
  return (
    <div className="space-y-6">
            ...
            <tbody className="divide-y divide-ink-100">
              {summary.map((row) => (
                <tr key={`${row.architecture}-${row.strategy_label}`}>
                  ...
                  <td className="td text-xs text-ink-500">{row.strategy_label}</td>
      ...
      <section className="space-y-2">
        {runs.length === 0 && (
          <p className="card p-4 text-sm text-ink-400">
        ...
        {runs.map((run) => (
          <div key={run.id} className="card overflow-hidden">
            ...
            >
              <span className="w-10 shrink-0 font-mono text-xs text-ink-400">#{run.id}</span>
              <span className="min-w-0 flex-1">
                <span className="block truncate text-sm font-medium text-ink-900">{run.question}</span>
            ...
            {selected === run.id && (
              <div className="space-y-4 border-t border-ink-200 bg-ink-50 px-4 py-4">
                {!detail && <p className="text-sm text-ink-400">Loading…</p>}
                {detail && (
                  <>
                    <div>
                    ...
                    <div>
                      ...
                      <pre className="overflow-auto rounded bg-white p-2 font-mono text-[11px] text-ink-700">
